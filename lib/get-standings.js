...
module.exports = function(season, matches) {
  ...
  season.weeks
    ...
    .forEach(match => {
      ...
      if (! match.isDone() ) {
        ...
        hr.points += p.home;
        ar.points += p.away;
        var pointTotal = p.home + p.away;
        if(pointTotal>0) {
          hr.pctTotal += p.home / pointTotal;
          ar.pctTotal += p.away / pointTotal;
        }
        hr.pops = formatPops(hr.pctTotal/hr.countOfMatches)
        ar.pops = formatPops(ar.pctTotal/ar.countOfMatches)
      }
    });
  ...
  standingsGroups.forEach(group => {
    group.sort((a, b) => {
      ...
      if(a.pctTotal > b.pctTotal) { return -1; }
      if(b.pctTotal > a.pctTotal) { return  1; }
